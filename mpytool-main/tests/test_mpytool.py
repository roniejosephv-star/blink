...
class TestArgsCommand(unittest.TestCase):
    ...
    def test_args_run(self, mock_stdout):
        """_args run shows an optional local_file argument ('-' = STDIN)"""
        self.tool.process_commands(['_args', 'run'])
        output = mock_stdout.getvalue()
        self.assertIn('local_file:?:', output)
...
class TestRunCommand(unittest.TestCase):
    ...
    def test_run_missing_argument(self):
        """'run' without file on an interactive terminal -> ParamsError"""
        # Force interactive stdin (TTY) so no-arg run does not read STDIN
        fake_stdin = Mock()
        fake_stdin.isatty.return_value = True
        with patch('mpytool.mpytool._sys.stdin', fake_stdin):
            with self.assertRaises(ParamsError) as ctx:
...
class TestRunStdin(unittest.TestCase):
    """Tests for run reading code from STDIN ('-' or piped)"""

    def setUp(self):
        self.tool = MpyTool(Mock(), verbose=None)
        self.tool._mpy = Mock()
        self.tool._log = Mock()

    def _fake_stdin(self, data, isatty):
        """Build a fake sys.stdin whose .buffer.read() yields data"""
        fake = Mock()
        fake.isatty.return_value = isatty
        fake.buffer.read.return_value = data
        return fake

    def test_run_dash_reads_stdin(self):
        """'run -' reads code from STDIN regardless of TTY state"""
        fake = self._fake_stdin(b"print('pipe')\n", isatty=True)
        with patch('mpytool.mpytool._sys.stdin', fake):
            self.tool.process_commands(['run', '-'])
        args, _ = self.tool._mpy.comm.try_raw_paste.call_args
        self.assertEqual(args[0], b"print('pipe')\n")

    def test_run_no_arg_piped_reads_stdin(self):
        """'run' with piped STDIN (not a TTY) reads code from STDIN"""
        fake = self._fake_stdin(b"x = 1\n", isatty=False)
        with patch('mpytool.mpytool._sys.stdin', fake):
            self.tool.process_commands(['run'])
        args, _ = self.tool._mpy.comm.try_raw_paste.call_args
        self.assertEqual(args[0], b"x = 1\n")

    def test_run_no_arg_tty_errors(self):
        """'run' with no arg on a TTY (no pipe) -> ParamsError"""
        fake = self._fake_stdin(b"", isatty=True)
        with patch('mpytool.mpytool._sys.stdin', fake):
            with self.assertRaises(ParamsError):
                self.tool.process_commands(['run'])

    def test_run_dash_fire_and_forget(self):
        """'run - -t 0' reads STDIN and fires without waiting"""
        fake = self._fake_stdin(b"boot()\n", isatty=False)
        with patch('mpytool.mpytool._sys.stdin', fake):
            self.tool.process_commands(['run', '-', '-t', '0'])
        self.tool._mpy.comm.try_raw_paste.assert_called_once_with(
            b"boot()\n", timeout=0)


class TestStdinConsumers(unittest.TestCase):
    """Tests for the STDIN single-consumption pre-flight check"""

    def _count(self, groups):
        from mpytool.mpytool import _count_stdin_consumers
        return _count_stdin_consumers(groups)

    def test_run_file_arg_extraction(self):
        """_run_file_arg finds the file positional past options"""
        from mpytool.mpytool import _run_file_arg
        self.assertEqual(_run_file_arg(['-']), '-')
        self.assertEqual(_run_file_arg(['-t', '0', '-']), '-')
        self.assertEqual(_run_file_arg(['app.py']), 'app.py')
        self.assertIsNone(_run_file_arg(['-t', '5']))
        self.assertIsNone(_run_file_arg([]))

    def test_reads_stdin_predicate(self):
        """_run_reads_stdin: '-' always, None only when piped"""
        from mpytool.mpytool import _run_reads_stdin
        self.assertTrue(_run_reads_stdin('-'))
        self.assertFalse(_run_reads_stdin('app.py'))
        with patch('mpytool.mpytool._sys.stdin') as stdin:
            stdin.isatty.return_value = False
            self.assertTrue(_run_reads_stdin(None))
            stdin.isatty.return_value = True
            self.assertFalse(_run_reads_stdin(None))

    def test_single_consumer_ok(self):
        """One stdin consumer is allowed"""
        self.assertEqual(self._count([['run', '-'], ['reset'], ['monitor']]), 1)

    def test_run_and_repl_conflict(self):
        """run - plus repl are two consumers"""
        self.assertEqual(self._count([['run', '-'], ['repl']]), 2)

    def test_two_runs_conflict(self):
        """Two piped runs are two consumers"""
        self.assertEqual(self._count([['run', '-'], ['run', '-']]), 2)

    def test_file_run_not_consumer(self):
        """run with a real file does not consume stdin"""
        self.assertEqual(self._count([['run', 'app.py'], ['repl']]), 1)

    def test_exec_and_monitor_not_consumers(self):
        """exec and monitor never consume stdin"""
        self.assertEqual(self._count([['exec', 'x=1'], ['monitor']]), 0)
...
class TestSyncCommand(unittest.TestCase):
    """Tests for sync command (deploy project per .mpyproject)"""

    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()
        self.tool = MpyTool(Mock(), verbose=None)
        # Capture cp invocations and the CWD active at call time
        self.cp_calls = []
        self.tool.cmd_cp = lambda args: self.cp_calls.append(
            (os.path.basename(os.getcwd()), list(args)))

    def tearDown(self):
        shutil.rmtree(self.tmpdir, ignore_errors=True)

    def _write_project(self, content, subdir=''):
        """Create a .mpyproject with given content, return its directory"""
        root = os.path.join(self.tmpdir, subdir) if subdir else self.tmpdir
        os.makedirs(root, exist_ok=True)
        with open(os.path.join(root, '.mpyproject'), 'w') as file:
            file.write(content)
        return root

    def test_default_deploy(self):
        """Missing deploy key -> default './' to root"""
        root = self._write_project('{"name": "demo"}')
        self.tool.cmd_sync(root)
        self.assertEqual([args for _, args in self.cp_calls], [['./', ':']])

    def test_deploy_dir_and_rename(self):
        """Dir mode list + rename mode string translate to cp jobs"""
        root = self._write_project(
            '{"deploy": {"/lib/": ["a.py", "b.py"], "/main.py": "src/m.py"}}')
        self.tool.cmd_sync(root)
        self.assertEqual(
            [args for _, args in self.cp_calls],
            [['a.py', 'b.py', ':/lib/'], ['src/m.py', ':/main.py']])

    def test_compile_flag(self):
        """compile:true prepends -m to every cp job"""
        root = self._write_project(
            '{"compile": true, "deploy": {"": ["./"]}}')
        self.tool.cmd_sync(root)
        self.assertEqual(self.cp_calls[0][1], ['-m', './', ':'])

    def test_dedupe_sources(self):
        """Duplicate sources in dir mode are removed, order preserved"""
        root = self._write_project(
            '{"deploy": {"": ["./", "./", "x.py"]}}')
        self.tool.cmd_sync(root)
        self.assertEqual(self.cp_calls[0][1], ['./', 'x.py', ':'])

    def test_exclude_merged(self):
        """exclude patterns from config merge into instance excludes"""
        root = self._write_project(
            '{"deploy": {"": ["./"]}, "exclude": ["*.bak", "tmp"]}')
        self.tool.cmd_sync(root)
        self.assertIn('*.bak', self.tool._exclude_dirs)
        self.assertIn('tmp', self.tool._exclude_dirs)

    def test_trailing_comma_tolerated(self):
        """JSON with trailing commas still loads"""
        root = self._write_project('{"deploy": {"": ["./"],},}')
        self.tool.cmd_sync(root)
        self.assertEqual(self.cp_calls[0][1], ['./', ':'])

    def test_runs_from_project_root(self):
        """cp jobs run with project root as CWD; CWD restored afterwards"""
        root = self._write_project('{"deploy": {"": ["./"]}}', subdir='myproj')
        prev = os.getcwd()
        self.tool.cmd_sync(root)
        self.assertEqual(self.cp_calls[0][0], 'myproj')
        self.assertEqual(os.getcwd(), prev)

    def test_search_upward(self):
        """.mpyproject is found by searching parent directories"""
        root = self._write_project('{"deploy": {"": ["./"]}}', subdir='app')
        deep = os.path.join(root, 'pkg', 'mod')
        os.makedirs(deep)
        self.tool.cmd_sync(deep)
        self.assertEqual(self.cp_calls[0][0], 'app')

    def test_no_mpyproject(self):
        """Missing .mpyproject raises ParamsError"""
        empty = os.path.join(self.tmpdir, 'empty')
        os.makedirs(empty)
        with self.assertRaises(ParamsError) as ctx:
            self.tool.cmd_sync(empty)
        self.assertIn('.mpyproject', str(ctx.exception))

    def test_path_not_found(self):
        """Nonexistent path raises ParamsError"""
        with self.assertRaises(ParamsError) as ctx:
            self.tool.cmd_sync(os.path.join(self.tmpdir, 'nope'))
        self.assertIn('not found', str(ctx.exception))

    def test_invalid_json(self):
        """Malformed JSON raises ParamsError"""
        root = self._write_project('{ not valid }')
        with self.assertRaises(ParamsError) as ctx:
            self.tool.cmd_sync(root)
        self.assertIn('invalid JSON', str(ctx.exception))

    def test_dir_mode_string_invalid(self):
        """Directory dest with string source is a config error"""
        root = self._write_project('{"deploy": {"/lib/": "single.py"}}')
        with self.assertRaises(ParamsError) as ctx:
            self.tool.cmd_sync(root)
        self.assertIn('must be a list', str(ctx.exception))

    def test_rename_mode_list_invalid(self):
        """Rename dest with list source is a config error"""
        root = self._write_project('{"deploy": {"/main.py": ["a.py"]}}')
        with self.assertRaises(ParamsError) as ctx:
            self.tool.cmd_sync(root)
        self.assertIn('must be a single string', str(ctx.exception))

    def test_nothing_to_deploy(self):
        """Deploy entries with only empty sources -> nothing to deploy"""
        root = self._write_project('{"deploy": {"/lib/": []}}')
        with self.assertRaises(ParamsError) as ctx:
            self.tool.cmd_sync(root)
        self.assertIn('nothing to deploy', str(ctx.exception))

    def test_dispatch_sync(self):
        """_dispatch_sync parses optional directory and consumes commands"""
        root = self._write_project('{"deploy": {"": ["./"]}}')
        commands = [root]
        self.tool._dispatch_sync(commands, True)
