...
_CMD_ORDER = [
    'ls', 'tree', 'cat', 'cp', 'sync', 'mv', 'mkdir', 'rm', 'pwd', 'cd', 'path',
...
def _find_mpyproject(path):
    """Find .mpyproject searching upward from path (file or directory).

    Mirrors the Sublime plugin: walks parent directories, skipping any
    .backup directory (which may hold a .mpyproject downloaded from a
    device). Returns the path to .mpyproject or None.
    """
    path = _os.path.abspath(path)
    directory = path if _os.path.isdir(path) else _os.path.dirname(path)
    while directory and directory != _os.path.dirname(directory):
        if _os.path.basename(directory) == '.backup':
            directory = _os.path.dirname(directory)
            continue
        candidate = _os.path.join(directory, '.mpyproject')
...
def _load_mpyproject(path):
    """Load .mpyproject JSON config.

    Tolerates trailing commas (common, not strictly valid JSON), like the
    Sublime plugin. Returns the parsed dict, {} for an empty file, or None
    ...
    if not content:
        return {}
    # Remove trailing commas before } or ] (tolerated, not valid JSON)
...
class MpyTool():
    ...
    def cmd_repl(self):
        ...
        terminal.run()
        # Drop keystrokes the device never consumed, so close() doesn't block
        # waiting to drain the kernel TX buffer.
        self.conn.discard_output()
    ...
    def cmd_ota(self, firmware_path):
        ...
        self.verbose(
            "  OTA complete! Use 'reset --machine' to boot into new firmware.",
            1, color='green')

    def cmd_ota_confirm(self):
        """Mark the running OTA app valid (cancel pending rollback)"""
        self.verbose("OTA CONFIRM", 1)
        running = self.mpy.ota_confirm()
        self.verbose(
            f"  {running} marked valid - rollback cancelled.",
            1, color='green')

    def cmd_ota_boot(self, label=None):
        """Switch the boot partition for the next reset"""
        self.verbose("OTA BOOT", 1)
        target = self.mpy.ota_set_boot(label)
        self.verbose(
            f"  Boot set to {target}. Use 'reset --machine' to boot it.",
            1, color='green')
    ...
    def _cmd_flash_esp32(self):
        ...
        # Warn if the boot-selected OTA app is awaiting confirmation
        pend = next(
            (p for p in info['partitions']
                if p.get('ota_boot') and p.get('ota_state') == 'pending-verify'),
            None)
        if pend:
            self._log.warning(
                f"{pend['label']} is PENDING_VERIFY - it will roll back on "
                "next reset unless the running app confirms it via "
    ...
    def cmd_sync(self, project_dir=None):
        """Deploy a project to the device per its .mpyproject config.

        Reads the deploy map / exclude / compile settings from .mpyproject
        (found in project_dir or the current directory, searched upward) and
        runs the corresponding cp operations. Non-interactive: the connection
        comes from the global -p / -a options.
        """
        start = project_dir if project_dir else '.'
        if not _os.path.exists(start):
            raise ParamsError(f"path not found: {start}")
        mpyproject = _find_mpyproject(start)
        if not mpyproject:
            raise ParamsError(
                f"no .mpyproject found in '{start}' or any parent directory")
        config = _load_mpyproject(mpyproject)
        if config is None:
            raise ParamsError(f"invalid JSON in {mpyproject}")
        root = _os.path.dirname(mpyproject) or '.'
        name = config.get('name') or _os.path.basename(_os.path.abspath(root))
        self.verbose(f"SYNC: {name} ({mpyproject})", 1)
        # Merge config exclude patterns into the active exclude set
        for pattern in config.get('exclude', []):
            self._exclude_dirs.add(pattern)
        compile_mpy = config.get('compile', False)
        # deploy map: {"": ["./"], "/lib/": ["../x.py"]}
        # dir mode (dest is "" or ends with "/"): sources is a list
        # rename mode (dest does not end with "/"): sources is a single string
        deploy = config.get('deploy') or {"": ["./"]}
        cp_jobs = self._build_sync_jobs(deploy, mpyproject)
        if not cp_jobs:
            raise ParamsError(
                f"nothing to deploy, configure 'deploy' in {mpyproject}")
        # Sources are relative to the project root - run cp from there
        prev_cwd = _os.getcwd()
        try:
            _os.chdir(root)
            for sources, dest in cp_jobs:
                cp_args = ['-m'] if compile_mpy else []
                cp_args.extend(sources)
                cp_args.append(dest)
                self.cmd_cp(cp_args)
        finally:
            _os.chdir(prev_cwd)
    ...
    @staticmethod
    def _build_sync_jobs(deploy, mpyproject):
        """Translate a .mpyproject deploy map into (sources, dest) cp jobs"""
        jobs = []
        for dest, sources in deploy.items():
            ...
            if is_dir_mode and not isinstance(sources, list):
                raise ParamsError(
                    f"deploy '{dest}' is a directory, sources must be a list "
                    f"(in {mpyproject})")
            if not is_dir_mode and not isinstance(sources, str):
                raise ParamsError(
                    f"deploy '{dest}' is a rename, sources must be a single "
                    f"string (in {mpyproject})")
            if is_dir_mode:
                # Drop empties, dedupe while preserving order
                srcs = list(dict.fromkeys(s for s in sources if s))
            else:
                srcs = [sources]
            if not srcs:
                continue
            jobs.append((srcs, f":{dest}"))
        return jobs
    ...
    @command('run', "Run local Python file on device ('-' = STDIN).")
    @argument('file', nargs='?', metavar='local_file',
        help="local .py file, or '-' for STDIN (also STDIN if piped)")
    @option('-t', '--timeout', type=float, default=None,
        help='timeout in seconds (0 = fire-and-forget)')
    def _dispatch_run(self, commands, is_last_group):
        args = _make_parser(self._dispatch_run).parse_args(commands)
        commands.clear()
        if _run_reads_stdin(args.file):
            code = _sys.stdin.buffer.read()
            self.verbose(f"RUN: <stdin> ({len(code)} bytes)", 1)
        elif args.file is None:
            raise ParamsError("run needs a file, '-', or piped STDIN")
        else:
    ...
    def _dispatch_flash(self, commands, is_last_group):
        ...
        flash_ota = flash_sub.add_parser(
            'ota', help='OTA firmware update / confirm / boot')
        flash_ota.add_argument(
            'args', nargs='+', metavar='arg',
            help="firmware .app-bin file, or 'confirm' (mark running app "
                 "valid), or 'boot [label]' (switch boot partition)")
        ...
        if args.operation == 'read':
            ...
            if args.args[0] == 'confirm':
                if len(args.args) != 1:
                    raise ParamsError('flash ota confirm (takes no args)')
                self.cmd_ota_confirm()
            elif args.args[0] == 'boot':
                if len(args.args) > 2:
                    raise ParamsError('flash ota boot [label]')
                label = args.args[1] if len(args.args) == 2 else None
                self.cmd_ota_boot(label)
            else:
                if len(args.args) != 1:
                    raise ParamsError('flash ota <firmware>')
                self.cmd_ota(args.args[0])
        else:
            self.cmd_flash()
    ...
    @command('sync', 'Deploy project to device per .mpyproject config.')
    @argument('dir', nargs='?', metavar='project_dir',
        help='project directory (default: current, searched upward)')
    def _dispatch_sync(self, commands, is_last_group):
        args = _make_parser(self._dispatch_sync).parse_args(commands)
        commands.clear()
        self.cmd_sync(args.dir)
    ...
    _COMMANDS = frozenset({
        ...
        'rtc', 'flash', 'sleep', 'cp', 'sync', 'mv', 'mount', 'ln', 'speedtest',
...
def _run_reads_stdin(file_arg):
    """True if a `run` with this file argument reads code from STDIN.

    '-' is explicit STDIN; a missing argument (None) reads STDIN only when it
    is piped (stdin is not an interactive terminal).
    """
    if file_arg == '-':
        return True
    if file_arg is None:
...
def _run_file_arg(group_args):
    """Extract the file positional from a `run` command group's arguments.

    group_args are the tokens after 'run'. Returns the file value ('-', a
    path, or None if absent). Must track run's value-taking options (-t).
    """
    parser = _argparse.ArgumentParser(add_help=False)
    parser.add_argument('-t', '--timeout')
    parser.add_argument('file', nargs='?')
    try:
        namespace, _ = parser.parse_known_args(group_args)
    except SystemExit:
        return None
    return namespace.file


def _count_stdin_consumers(command_groups):
    """Count command groups that would consume STDIN.

    STDIN is single-consumption: `run -` (or piped `run`) reads it all as a
    script, and `repl` needs it for the interactive terminal. More than one
    consumer in a single invocation is a conflict.
    """
    count = 0
    for group in command_groups:
        ...
        if group[0] == 'repl':
            ...
            if _run_reads_stdin(_run_file_arg(group[1:])):
                count += 1
    return count
...
def main():
    ...
    # STDIN can be consumed only once - reject multiple consumers up front
    if _count_stdin_consumers(command_groups) > 1:
        log.error(
            "only one command can read from STDIN "
            "(run - / piped run, repl)")
        _sys.exit(1)
    try:
